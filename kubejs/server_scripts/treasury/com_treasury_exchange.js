...
// ============================================================================
const EX_BILLS = [
    { id: 'utopia:500_dollar_bill', val: 1000 },
...
ServerEvents.commandRegistry(event => {
    ...
    const registerExchangeTree = (baseCommandName) => {
        ...
        // Base Command: /tres exchange -> Shows Info & Rates
        .executes(ctx => showExchangeInfo(ctx))

        // Admin Command: King Withdraws Paper Currency
        .then(C.literal('withdraw')
        .then(C.argument('amount', A.INTEGER.create(event))
        .executes(ctx => handleKingWithdraw(ctx, A.INTEGER.getResult(ctx, 'amount')))
        )
        )

        // Subcommand: /tres exchange rate -> Also shows Info & Rates
        .then(C.literal('rate')
        .executes(ctx => showExchangeInfo(ctx))
        )

        // Subcommand: /tres exchange set_rate <material> <rate>
        .then(C.literal('set_rate')
...
function showExchangeInfo(ctx) {
    ...
    let message = Text.gold('\n\n=== Treasury Paper Exchange ===\n')
    .append(Text.gray('Trade in your Paper Currency for Kingdom Assets.\n'))
    .append(Text.yellow('Usage: ').append(Text.white('/tres exchange <iron|diamond|scrap> <R$D_Amount>\n')))
    ...
    ['iron', 'diamond', 'scrap'].forEach(mat => {
        ...
        if (rate > 0) {
            message.append(Text.green(`[ ${rate} R$D ] `).append(Text.white(`= 1 ${label} Coin\n`)));
        } else {
            message.append(Text.red(`[ Unpegged ] `).append(Text.gray(`- ${label} is not trading.\n`)));
        }
    });

    let vaultCash = safeNum(server.persistentData.tres_paper_dollars, 0);
    message.append(Text.gold('\n--- Vault Cash Reserves ---\n'))
    .append(Text.green(`[ ${vaultCash} R$D ] `).append(Text.white('currently held in the Treasury.')));
...
function handleTradeIn(ctx, rawMat, rsdAmount) {
    ...
    // Log the actual R$D paper funds entering the treasury
    let vaultCash = safeNum(server.persistentData.tres_paper_dollars, 0);
    server.persistentData.tres_paper_dollars = vaultCash + rsdAmount;
...
function handleKingWithdraw(ctx, amount) {
    const player = ctx.source.player;
    const server = ctx.source.server;

    if (player.username !== server.persistentData.current_king) {
        player.tell(Text.red('Only the King may withdraw cash reserves directly from the Treasury.'));
        return 0;
    }

    if (amount <= 0) {
        player.tell(Text.red('Withdrawal amount must be greater than zero.'));
        return 0;
    }

    let vaultCash = safeNum(server.persistentData.tres_paper_dollars, 0);
    if (vaultCash < amount) {
        player.tell(Text.red(`Transaction Cancelled: The Treasury only holds ${vaultCash} R$D in cash reserves.`));
        return 0;
    }

    // Calculate and dispense exact bills using the greedy approach (largest denominations first)
    let remainingToDispense = amount;
    for (let b of EX_BILLS) {
        let count = Math.floor(remainingToDispense / b.val);
        if (count > 0) {
            // Keep stacks manageable by splitting them if needed (though give() generally handles overflows fine)
            let remainingBills = count;
            while (remainingBills > 0) {
                let chunk = Math.min(remainingBills, 64);
                player.give(Item.of(b.id, chunk));
                remainingBills -= chunk;
            }
            remainingToDispense -= count * b.val;
        }
    }

    // Deduct funds from the treasury ledger
    server.persistentData.tres_paper_dollars = vaultCash - amount;

    player.tell(Text.green(`Successfully withdrew ${amount} R$D from the Treasury cash reserves.`));
    return 1;
}
